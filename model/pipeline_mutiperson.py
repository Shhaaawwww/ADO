...
class CatVTONPipeline:
    def __init__(
        ...
        self.lpips_model = lpips.LPIPS(net="vgg").to(device)  # 初始化LPIPS模型
        # Pytorch 2.0 Compile
        if compile:
    ...
    def _run_diffusion_with_grad(
        ...
        # 去噪循环，使用更简单的gradient checkpointing策略
        ...
        progress_bar = tqdm(total=len(timesteps), desc="扩散进度", ncols=80)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Prepare timesteps
        self.noise_scheduler.set_timesteps(num_inference_steps, device=self.device)
        timesteps = self.noise_scheduler.timesteps  # 获取时间步序列
        latents = (
            latents * self.noise_scheduler.init_noise_sigma
        )  # 缩放初始噪声self.noise_scheduler.init_noise_sigma = 1
        # Classifier-Free Guidance
        if do_classifier_free_guidance := (guidance_scale > 1.0):
            masked_latent_concat = torch.cat(
                [
        ...
        with tqdm(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # latents1是使用原图像和模特结合后产生的，在优化迭代中用不上
        ...
        return image0, image1  # 0是目标，1是原始
    ...
    def attack(
        ...
        **kwargs,
        ...
        print(f"处理 {num_models} 个模特进行优化")

        # 只保存原始图像和mask的引用，不进行VAE编码
        processed_images = []
        processed_masks = []
        
        for i in range(num_models):
            # 提取单个模特的图像和mask - 正确的维度处理
        ...
        def get_model_data_and_target(model_idx):
            ...
            with torch.no_grad():
                ...
                # 每次生成新的固定噪声（使用相同的generator seed保证一致性）
                if generator is not None:
                    generator.manual_seed(model_idx + 12345)  # 固定种子确保噪声一致
                ...
                # 每次重新计算目标结果
                print(f"重新计算模特 {model_idx + 1} 的目标结果...")
        ...
        for i in range(attack_steps):
            ...
            print(f"步骤 {i+1}/{attack_steps}: 使用模特 {current_model_idx + 1}/{num_models}")
            ...
            with torch.no_grad():
                ...
                swanlab.log({
                    ...
                    'LPIPS_targetcloth_vs_origincloth': LPIPS_targetcloth_vs_origincloth.item(),
                    'LPIPS_optcloth_vs_origincloth': LPIPS_optcloth_vs_origincloth.item(),
                    'SSIM_optcloth_vs_origincloth': SSIM_optcloth_vs_origincloth.item(),
                    'SSIM_targetcloth_vs_origincloth': SSIM_targetcloth_vs_origincloth.item(),
                    'LPIPS_optresult_vs_targetresult': LPIPS_optresult_vs_targetresult.item(),
                    'SSIM_optresult_vs_targetresult': SSIM_optresult_vs_targetresult.item(),
                    'clip_opt_result': clip_opt_result,
                    "Image/optcloth": swanlab.Image(condition_image0, caption="optimized_image"),
                    "Image/Conditionwear": swanlab.Image(result_image0, caption="current_result_image"),
                    "Image/targetcloth": swanlab.Image(target_condition_image0, caption="target_condition_image"),
                    "Image/refcloth": swanlab.Image(ref_condition_image0_log, caption="ref_condition_image"),
                    "Image/targetresult": swanlab.Image(targetresult_image0, caption="target_result_image"),
                })
            
            print(f"步骤 {i+1}/{attack_steps}: 模特{current_model_idx+1}, 总损失 = {total_loss_item:.6f}, 相似度损失 = {loss1_item:.6f}, 正则化损失 = {loss2_item:.6f}")
