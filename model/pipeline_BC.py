...
#666
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Process final latent vectors to get final images
    ...
    def attack(
        ...
        image: Union[PIL.Image.Image, torch.Tensor],
        condition_image: Union[PIL.Image.Image, torch.Tensor],
        ref_condition_image: Union[PIL.Image.Image, torch.Tensor],  # Original unoptimized condition_image for regularization
        target_condition_image: Union[PIL.Image.Image, torch.Tensor],
        mask: Union[PIL.Image.Image, torch.Tensor],
        num_inference_steps: int = 50,
        guidance_scale: float = 2.5,
        ...
        patience: int = 10,  # New: how many steps of stagnation before reducing learning rate
        **kwargs,
    ):
        ...
        print(f"At start ref_latent: requires_grad={condition_latent.requires_grad},min={condition_latent.min().item():.6f}, max={condition_latent.max().item():.6f},norm={condition_latent.norm().item():.6f}")
        ...
        def calc_clip_similarity(img_tensor1, img_tensor2):
            ...
            def tensor_to_pil(tensor):
                # Ensure tensor is on CPU and is float32
                if tensor.is_cuda:
                    tensor = tensor.cpu()
        ...
        for i in range(attack_steps):
            ...
            cos_sim2 = torch.nn.functional.cosine_similarity(condition_latent, ref_condition_latent)
            loss2 = (1 - cos_sim2).mean()
